...
class Bot {
   ...
   * lower case, or "logged out" if {@linkcode Bot#start|start()} has not been
   * called or {@linkcode Bot#stop|stop()} has been called and the bot has been
   * stopped.
   */
  get status() {
   ...
   * the user. The default implementation returns `"Bot"`.
   *
   * @return {string}
   *     the bot's name
   */
  ...
  handleMessage(message) {
    ...
    if (typeof lastMessage === 'number' && lastMessage + this._cooldown > now) {
      ...
      return this._catch(this.messageOnGlobalCooldown(message, lastMessage + this._cooldown));
    }

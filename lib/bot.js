...
/**
 * Utility function for looking up a path in multiple objects.
 */
function dig(o, keyPath, defaultValue) {
...
class Bot {
  constructor(defaultConfig) {
    this._client = null;
    this._config = null;
    this._defaultConfig = (defaultConfig !== null && typeof defaultConfig === 'object') ? defaultConfig : {};
  ...
  /**
   * Gets a configuration option for the given key. The key can either be a
   * dot-separated list of names (like "foo.bar") or an array (like ["foo",
   * "bar"]).
   */
  getConfig(key, defaultValue) {
    ...
    if (typeof key === 'string')
      key = key.split('.');
    // It's possible for user-specific config to not be set. This will hopefully
    // cause things to blow up.
    let result = this._config ? dig(this._config, key, null) : null;
    if (result === null) {
      if (arguments.length < 2) {
        return dig(this._defaultConfig, key);
      } else {
        return dig(this._defaultConfig, key, defaultValue);
      }
    } else {
      return result;
    }
   ...
   * Bot when the configuration is loaded.
   */
  loadConfig(filename) {
  ...
  handleCommand(command, message) {
    if (/^\s*$/.test(command)) {
      ...
      return this.blankCommand(command, message);
    }
    ...
    if (typeof commandObj === 'object') {
      ...
      this.unknownCommand(command, args, message);
    }
  }
  /**
  ...
  unknownCommand(command, args, message) {
    return message.channel.send(`Unknown command "${command}".`).catch(error => {
      this.handleError(error, "Unable to send message:");
    });
  }
  /**
   * Handle a blank command. The default implementation does nothing.
   */
 ...
 * "bot hello" and "  bot   hello" would trigger the bot but "bothello" would
 * not.
 */
...
// Re-export the CLICommand class

...
class StatusCommand extends CLICommand {
  ...
  run(cli, args) {
    ...
    let user = cli.bot.user;
    if (user) {
      cli.writeLine(`Logged in as ${user.username}#${user.tag} (${user.id})`);
    }
...
class JoinCommand extends CLICommand {
  constructor() {
    super("join", "launches the bot join URL in a browser");
  }
  run(cli, args) {
    if (open !== null) {
      open(cli.bot.getBotJoinURL()).catch(error => {
        cli.writeLine("Unable to launch join URL: " + error);
      });
    } else {
      cli.writeLine("open is not installed: install it with your Node package manager to use");
    }
  }
}
...
if (open !== null) {
  exports.DEFAULT_COMMANDS.push(JoinCommand);
}

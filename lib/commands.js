 ...
 *
 * Note that you don't need to extend this class to create a new command, but
 * this class provides helpful stubs. Strictly speaking the only function your
 * command object needs to run is the {@link BasicCommand#run run} function.
 *
 * However, such an object (with only a `run` function) cannot be added via
 * {@link Bot#addCommands} and can only be added via {@link Bot#addCommand} with
 * a given set of names.
 *
 * Likewise, any command without a `name` property and without a
 * {@link BasicCommand#getHelp} function will not appear in the list of commands
 * displayed by {@link HelpCommand}.
 */
class BasicCommand {
  /**
   * Create the command.
...
class HelpCommand extends BasicCommand {
  ...
  run(bot, args, message, line) {
    ...
    // TODO: Aggregate command aliases such that a command will be grouped with
    // all its aliases ... somehow
    for (let [command, obj] of bot._commands.entries()) {
      // If there is no getHelp, this command is "hidden"
      if (typeof obj.getHelp === 'function') {
        let help = obj.getHelp(locale);
        if (help) {
          m.push('  \u2022 **' + discord.escapeMarkdown(command) + '** \u2014 ' + help);
        }
      }
    }
    if (this.epilogue) {
